...
bool MatDetectorFrontCamera::getMatContour(std::vector<std::vector<cv::Point>>& contours, const cv::Mat& src) {
...
std::vector<cv::Vec4f> MatDetectorBottomCamera::findHorizontalLines(const cv::Mat& image, std::vector<std::vector<float>>& angle, std::vector<cv::Vec4f>& lines) {
    ...
    */
    ...
    for (int i = lines.size() - 1; i >= 0; i--) {
        if ((!lines.empty()) && ((angle [1][i] < min_angle) || (abs(abs(angle [1][i]) - abs(min_angle)) == parallel_criteria)) && (angle [1][i] < 15)) {
            if (getLength(lines[angle [0][i]]) > (image.cols/2)) {
                min_angle = angle [1][i];
...
std::vector<cv::Vec4f> MatDetectorBottomCamera::findVerticalLines(const cv::Mat& image, std::vector<std::vector<float>>& angle, std::vector<cv::Vec4f>& lines) {
    ...
    for (int i = 0; i <= lines.size()-1; i++) {
        if ((!lines.empty()) && ((angle [1][i] > max_angle) || (abs(abs(angle [1][i]) - abs(max_angle)) == parallel_criteria)) && (angle [1][i] > 80)) {
            if (getLength(lines[angle [0][i]]) > (image.rows/2)) {
                max_angle = angle [1][i];
