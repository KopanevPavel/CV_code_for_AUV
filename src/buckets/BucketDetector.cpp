...
void BucketDetector::defaultPreprocess(const cv::Mat &src, cv::Mat &dst) {
    // TODO Define which pre-processing methods should be in common scope
    dst = createPipeline(src, false)
            //.apply(std::bind(&BucketDetector::meanShift, this, std::placeholders::_1, std::placeholders::_2), "MeanShift")
            .apply(std::bind(&BucketDetector::extractValueChannel, this, std::placeholders::_1, std::placeholders::_2), "Value channel")
            .apply(std::bind(&BucketDetector::morphology, this, std::placeholders::_1, std::placeholders::_2), "Rid of noise")
            .getImage();
}

void BucketDetector::meanShift(const cv::Mat &src, cv::Mat &dst) {
...
void BucketDetector::extractValueChannel(const cv::Mat &src, cv::Mat &dst) {
    cv::Mat hsv;
...
void BucketDetector::morphology(const cv::Mat &src, cv::Mat &dst) {
...
std::vector<cv::Vec3f> BucketDetector::findCircles(const cv::Mat& src, cv::Mat& dst) {
    std::vector<cv::Vec3f> circles;

    cv::Mat gray, image_copy;
    image_copy = src;
    ...
    */
    ...
    cv::addWeighted(hsv, 1, src_copy, 1, 0, src_copy); /// Sum to increase bucket color intensity
...
void pick_red_bucket_color(int event, int x, int y, int f, void *)
{
    if (event==1)
    {
        int r = 3;
...
/// Mouse callback function (returns color of the place where you clicked)
void pick_blue_bucket_color(int event, int x, int y, int f, void *)
{
    if (event==1)
    {
        int r = 3;
...
bool BucketDetector::isRed(const cv::Mat& src, const cv::Vec3f circle) {
    ...
    cv::setMouseCallback("Pick Red", pick_red_bucket_color, 0);
...
bool BucketDetector::isBlue(const cv::Mat& src, const cv::Vec3f circle) {
    ...
    cv::setMouseCallback("Pick Blue", pick_blue_bucket_color, 0);
...
BucketDescriptor BucketDetector::detect(const cv::Mat src, bool withPreprocess) {
    cv::Mat image;
    if (withPreprocess) {
        /*
